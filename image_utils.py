...
def read_ppm(filename):
    ...
    :return: a list-of-lists representing the image. The list-of-lists will be width x height,
    and each element will be a 3-tuple representing the color of the image as a red / green / blue value.
    """
    f = open(filename, "r")
    file = f.read().split()
    f.close()
    ...
    for columnNum in range(width):
        row = [BLACK] * height
        image.append(row)
    # a width x height list of lists (that is a width long list,
    # each element of which is a height lon lists) of colors
    i = 0
